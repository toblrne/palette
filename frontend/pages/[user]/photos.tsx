  ...
  useEffect(() => {
    if (inView && hasMore) {
      const fetchPosts = async () => {
        try {
          const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/posts/user/${router.query.user}?skip=${(page - 1) * 9}&limit=9`);
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const postsRes = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/posts/user/${userId}?skip=0&limit=9`);
